def main(hit, scene, output):
	...
	for light in scene.lights:
		...
		if ndl <=0:
			continue
		if inshadow == False:
			col = col + ((difk*dcol/3.1415926)+(rdwo**glossy)*sepccol)* (ndl*light.G(hit)* light.L(hit, scene)/light.pdf(hit))
	...
	col = col + Vector3.dot(hit.normal, vrefl) * fr * scene.tracer.trace(reflray, scene, 0.000001, hit.depth+1)
